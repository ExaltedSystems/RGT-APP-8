...
import { SecondsPipePipe } from 'src/app/pipes/seconds-pipe.pipe.pipe';
...
export class FlightsListingComponent implements OnInit {
      ...
      this.__ms.postData(this.flightsUrl, this.frmObj).subscribe(res => {
        ...
        if (res != null) {
          ...
          this.availableFlights.forEach((element, index) => {
            ...
            if(index == 0){
              this.options.floor = lowestAmount;
              this.minPrice = lowestAmount;
            }
            this.options.ceil = lowestAmount;
  ...
  flightDetailsToggle(ev, other?) {
    ...
    if(other == 'FBD'){
      jQuery('#FareBreakdownDetails-'+currId).toggleClass('show');
      ...
      jQuery('#showFlightDetails-'+currId).toggleClass('show');
