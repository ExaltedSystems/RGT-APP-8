...
import allAirlinesList from 'src/assets/js/locations.json';
...
export class FlightsSearchComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.flightSearch = this.__fb.group({
			...
			infant: [this.infant],
			clientPhone: ['', [Validators.minLength(10), Validators.maxLength(19), Validators.pattern(this.__ms.phonePattern)]]
		});
		let pageUrl = this.__router.url.substr(1);
		if (pageUrl.split('/')[0] == 'airlines') {
			this.flightSearch.controls['clientPhone'].setValidators([Validators.required, Validators.minLength(10), Validators.maxLength(19), Validators.pattern(this.__ms.phonePattern)]);
