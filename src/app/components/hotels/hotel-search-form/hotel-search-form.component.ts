...
export class HotelSearchFormComponent implements OnInit {
  ...
  private _cookieService: CookieService) { 
    ...
    let finalYear = new Date(yr + 2, month, day);
    this.maxDate = finalYear;
  ...
  closeDropDown(ev) {
    ...
    if(type == 'Check-In Date') {
      let dt = new Date(this.hotelSearch.controls['checkInDate'].value);
      let year = dt.getFullYear();
      let month = dt.getMonth();
      let day = dt.getDate();
      this.checkOutMinDate = new Date(year, month, day + 1);
    }
		// if (isObject(evt)) {
