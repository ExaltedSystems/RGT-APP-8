...
export class PnrViewComponent implements OnInit {
  ...
  constructor(private __actRouter: ActivatedRoute, private __ms: MainService, 
    private __router: Router, @Inject(PLATFORM_ID) private platformId: Object) {
    // window.scroll(0, 300);
  }
  ...
  ngOnInit() {
    ...
    let flightInfoObj = {
      ...
      _token: (isPlatformBrowser(this.platformId) ? localStorage.getItem("paxToken") : '')
      // _flag: 'ccard'
    } 
    Object.assign({refrenceNo:this.refNo}, {token:(isPlatformBrowser(this.platformId) ? localStorage.getItem("paxToken") : '')})
