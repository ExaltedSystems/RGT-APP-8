...
export class HomeComponent implements OnInit {
  ...
  showHideSearchFormTabs(evt, itemId, tabId) {
    evt.preventDefault();
    jQuery(".search-form-tab").each(function(i, v) {
      let listItemId = jQuery(v).attr('id');
      let tabPaneId = jQuery(v).attr('href');
      if(listItemId != itemId) {
        jQuery(v).removeClass('active show');
      }
      if(tabPaneId != "#"+tabId) {
        jQuery(tabPaneId).removeClass('active show');
      }
    });
		jQuery('#'+itemId).toggleClass('active show');
		jQuery('#'+tabId).toggleClass('active show');
  }
