...
import { MatHorizontalStepper, DateAdapter, MAT_DATE_FORMATS } from '@angular/material';
...
export class CalculateUmrahPackageComponent implements OnInit {
  ...
  addMoreHotelRows(): FormGroup {
    return this.__fb.group({
      ID: [''],
      locations: ['Makkah', Validators.required],
  ...
  addMoreTourRows() {
    ...
    let totalRows = this.hotelRows.length;
    let formArray = this.umrahCalculatorForm.controls['hotelRows'] as FormArray;
    let formGroup = formArray.controls[totalRows - 1] as FormGroup;
    let lastCheckOut = formArray.controls[totalRows - 2] as FormGroup;
    let lastLocation = lastCheckOut.controls['locations'].value;
    ...
    this.ManageNameControl(totalRows - 1);
    this.calculateDate(totalRows - 1);
    if(lastLocation == "Makkah") {
      this.getHotelByLocation("Madinah", (totalRows - 1));
      formGroup.controls['locations'].setValue('Madinah');
    } else {
      this.getHotelByLocation("Makkah", (totalRows - 1));
      formGroup.controls['locations'].setValue('Makkah');
    }
  }
  ...
  getTourHotels() {
    this.__ms.getData(this.__ms.backEndUrl + 'Umrah/tourHotelAutocomplete').subscribe(result => {
      ...
      this.getHotelByLocation('Makkah', 0);
      // }
    })
  }
  ...
  // Function to Find Invalid Inputs
  validateUBCInputs() {
    // const invalid = [];
    // const controls = this.umrahCalculatorForm.controls;
    ...
    // }
    // return invalid;
    // let umrahCalculatorForm:FormGroup|FormArray;
    ...
    recursiveFunc(this.umrahCalculatorForm);
  ...
  getHotelByLocation(ev: any, key: number) {
    ...
    // if(typeof this.vehicleLists != 'undefined') {
    //   this.__setPaxVehicle();
    // }
    let location: string;
    if (isObject(ev)) {
  ...
  private __filterHotels(value: string, hotelLists, key): string[] {
    ...
    if (filterResult.length > 0) {
      ...
      let location = formGroup.controls['locations'].value;
  ...
  setTransport(ev) {
    // Commented code is for Select Option onChange function
    // let isT = ev.source.value;
    // this.isTransport = (isT == 1 ? true : false);
    // Below code is for Checkbox change event
    let isTransport = this.umrahCalculatorForm.controls['isTransport'].value;
    this.isTransport = (isTransport ? true : false);
  }
